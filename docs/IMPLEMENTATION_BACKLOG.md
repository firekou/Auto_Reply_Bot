...
第一個程式批次（ARB-002）只涵蓋 P1：離線回播與核心排程。不做完整控制台、不接真實供應商、不做直播。控制介面在 P1 只有 stdin 控制通道，圖形控制頁最早在 P4 之後再評估。

每個任務的驗收都必須是可重跑的命令或可檢查的檔案。沒有 fixture 或證據時必須明確失敗，不得 silent pass。

### 0.1 checkpoint 切分（R1 F5）

P1 不是一次交付。先交一個能跑的最小循環，再補其餘項目：

| checkpoint | 內容 | 交付條件 |
| --- | --- | --- |
| C1 最小循環 | 設定載入、mock 四件套、串行 director、驗證器、去重、預算保留、stdin 控制、JSONL 日誌、關鍵測試 | 一個命令可重播，能看到讀到什麼、選了哪則、回了什麼、何時模擬播放 |
| C2 回播規模 | 100 則以上唯一留言的場景、30 分鐘場景時間回播、覆蓋率與丟棄分類報告 | 可重跑且結果一致 |
| C3 其餘 P1 | runbook 3.2 其餘故障案例、`arb doctor` 骨架 | 未完成的項目如實標未完成 |

C1 的範圍不以 T-101 至 T-116 全部做滿為前提。未做的項目留在清單上並標未完成，不把 C1 當成 P1 全部驗收。

### 0.2 工期說明（R1 F5）

下面各節的工作量是**規劃用的粗估**，不是有實作根據的交期承諾。先前把 P1 至 P3 相加成「42 至 50 個工作天」的寫法已移除：那個數字會被誤讀為排程承諾。實際工期以完成的 checkpoint 重估。

後備路徑不進第一版展示的必要路徑。OCR、prompt caching、Live Studio、虛擬音訊裝置與第二套 provider 一律延後，除非選定的主路徑被證明不能運作。
...
P1 粗估約 14 至 16 個工作天，全部不需要直播設備。這是粗估，不是交期承諾。

對應 checkpoint：C1 涵蓋 T-101 至 T-113 的核心部分加 T-116 以外的關鍵測試；C2 涵蓋 T-114、T-115；C3 補其餘。

P1 全部通過的條件：30 分鐘場景時間回播完成，且 POC_RUNBOOK 第 3 節列出的故障案例全部可重現。任何一項以「環境不支援」略過都必須在報告中明列，不得計入通過。
...
| T-204 | OCR 後備路徑（條件任務，主路徑可行則不做） | T-203 | L | `src/chat/ocr.ts` | 誤讀率與誤去重率分開報告，不與 DOM 路徑合併計分 | 需直播電腦 |
...
| T-209 | prompt caching 評估（條件任務，延後至主路徑穩定後） | T-205 | M | `src/providers/claude.ts` | 開啟前後的 input token 與延遲對照表 | 需 API 金鑰 |
...
P2 粗估約 18 至 22 個工作天，非交期承諾。開始前必須先取得第 4 節列出的資訊，並先閉合 R1 的 F4 與 F6。

P2 的範圍收斂（R1 F5）：**一種**擷取、**一種**留言來源、**一個**模型、**一個** TTS、**一個**播放器。因此 T-204（OCR）與 T-209（prompt caching）移出 P2 主路徑，改列為條件任務，只有在主路徑被證明不能運作時才排入。P2 的目標交付是一段可聽見角色回應的 10 分鐘本機錄影。
...
| T-303 | 虛擬音訊裝置後備路徑驗證（條件任務，主路徑不相容才做） | T-301 | M | `docs/obs-setup.md` | 在 Application Audio Capture 不相容時可用 | 需直播電腦 |
| T-304 | Live Studio 路徑查證與驗證（延後，非第一版展示依賴） | T-302 | L | `docs/live-studio.md` | 先把設計文件 6.2 的四個 UNKNOWN 逐項查證；無法確認就維持 UNKNOWN | 需直播電腦與平台帳號 |
| T-305 | 受控試播與接收端延遲量測 | T-302 | M | `reports/` | 本機延遲與觀眾端延遲分開記錄 | 需直播權限，另行授權 |

P3 粗估約 10 至 12 個工作天，非交期承諾。試播需要獨立授權，本輪未包含。第一個真正的節目驗收是可聽見角色回應的本機錄影；mock 靜音檔只證明管線接通，不算節目成品（R1 F5）。
...
成本控制的機制是先預留後結算，細節在 IMPLEMENTATION_DESIGN 7.1.2。以特定參數算出的每小時數字是情境估算，不是上界（R1 F4）。SDK 層的自動重試必須顯式關閉，否則 director 的「不重試」形同虛設。
